...
def _Generate_moving_edge_frames(
    n_frames: int = 30,
) -> list[np.ndarray]:
...
def v2e_aedat4_path(tmp_path: Path) -> Path:
    ...
    Save_events(stream, aedat4_file, Format="aedat4", Width=WIDTH, Height=HEIGHT)
    return aedat4_file


@pytest.fixture
def v2e_pipeline() -> Aedat4Inference:
    """Pipeline configured for v2e synthetic resolution."""
    settings = Aedat4InferenceSettings(
    ...
    return Aedat4Inference(settings)
...
def test_v2e_full_pipeline_loads_and_infers(
    ...
    v2e_pipeline: Aedat4Inference,
) -> None:
    """Full pipeline: AEDAT4 → events → time-surfaces → keypoints."""
    stream = v2e_pipeline.Load_events_from_aedat4(v2e_aedat4_path)
